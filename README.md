...
`src/config/stimulus.ts` defines `maxWidthDelta`, currently `0.15`. Consequently the selected release endpoints produce a configurable ±15% change relative to the contour's half-width. The renderer changes mesh X coordinates only and resets target Y coordinates to their source values; vertical movement is outside the study invariant.
...
5. Confirm the two endpoints remain plausible and represent the selected configurable ±15% release range.
